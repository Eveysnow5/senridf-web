...
// 2026-08-13 基线首跑发现的失效形态：**结论跨轮硬化**。
// 第 2 轮模型明确写了"由于没有附注页，我只能报告这个数字的存在"；第 3 轮却硬化成
// "城投类地铁公司的其他收益绝大部分为政府补贴，因此存在生存级依赖"——那句话是外部
// 知识，文件里没有，而整个结论就架在它上面。**看到的证据没增加，确定性却增加了。**
test('最后一轮要压住"结论硬化"：因果判断必须指得到原文', () => {
  const last = buildRoundTail(5, 5);
  assert.ok(last.includes('因果判断'), '缺少对因果判断的约束');
  assert.ok(last.includes('未披露'), '缺少"指不到就写未披露"的替代写法');
  assert.ok(/这类公司通常|在会计实务中/.test(last), '缺少对行业常识式解释的具体点名');
  assert.ok(
...
test('催收敛与压硬化是两回事，别互相顶掉', () => {
  ...
  assert.ok(/因果判断/.test(buildRoundTail(5, 5)));
  assert.ok(!/因果判断/.test(buildRoundTail(2, 5)), '中间轮不必压硬化，它还会继续取证');
});
