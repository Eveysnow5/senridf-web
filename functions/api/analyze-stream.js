...
4. 每引用一个具体数字，必须在其后用括号标注来源（文件名·章节或页），如"营收 1.2 亿元（文件1·合并利润表）"；无法定位具体位置的，至少标注来源文件名。

分析要求：
- 分析开头先输出一张【关键指标概览】Markdown 表格：多份文件时列为各文件/主体、行为关键指标（营业收入、净利润、毛利率、同比增长率、总资产、经营现金流等，按文件实际披露选取），缺披露的填"⚠️未披露"；单份文件时用两列（指标 | 数值）。表格之后再展开文字分析。
- 用中文撰写，语言专业简洁
