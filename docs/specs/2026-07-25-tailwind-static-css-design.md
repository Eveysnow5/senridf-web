...
**第一层（自动化，硬门槛，接进 `npm run check` + CI）——用"重新生成后对比"实现**：

不自己写"提取每个 class 再逐个核对"的脚本（class 名到 CSS 选择器的转义规则繁琐、易出 bug，本项目 grep 假阴性就栽过）。改用更稳的做法：检查脚本重新跑一次 Tailwind 生成到**临时文件**，跟已提交的 `css/tailwind.min.css` 逐字节对比——**一致=已提交的 CSS 就是当前源码该有的样子；不一致=有人改了 class 却没重新生成，报错并提示跑 `npm run build:css`**。

- **为什么这样更好**：直接借用 Tailwind 自己的扫描器判断"用到的类变了没有"，不用重写一遍它的提取+转义逻辑，天然正确处理 JS 字符串里的类、方括号任意值、变体前缀（`hover:`/`sm:`）等所有情况。
- **它精准命中的失败模式**：给页面新增了当前没用过的类却忘了重新生成 → 对比不一致 → 报错。正是用户最怕的"忘了跑 → 静默破坏"。
- **论证强度**：Tailwind 运行时编译器和 CLI 静态生成对同一 class 产出同一条规则（同 3.4.16 版本），静态版功能等价于现在运行时编译的结果；加上"生成物 == 当前源码应有产物"的对比，正确性扎实。
- **实现要点**：① 生成输出须跨机器确定（版本已精确锁 3.4.16；对比前把两边换行统一成 LF，避免 Windows 开发机 CRLF 与 Linux CI LF 的假差异）；② CI 里 `npm ci` 会装上 tailwindcss（devDep），检查能跑；③ 残留的不可静态判定边角（源码里连字面量都没有的纯计算类）任何静态手段都无法覆盖，但本项目实际不存在这种（动态 bg 颜色都有字面量），不构成现实风险。
